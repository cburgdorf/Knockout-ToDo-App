...
(function() { 
    ...
    ko.templateSources.anonymousTemplate.prototype['text'] = function(/* valueToWrite */) {
        ...
        // ---- ko.templateSources.inMemory -----
    
        ko.templateSources.inMemory = function (template, templateStorage) {
            this.template = template;
            this.templateStorage = templateStorage;
        };

        ko.templateSources.inMemory.prototype['text'] = function (/* valueToWrite */) {
            if (arguments.length == 0) {
                //return this.domElement.tagName.toLowerCase() == "script" ? this.domElement.text : this.domElement.innerHTML;
                return this.templateStorage[this.template];
            } else {
                var valueToWrite = arguments[0];
                this.templateStorage[this.template] = valueToWrite;
            }
        };
...
ko.exportSymbol('ko.nativeTemplateEngine', ko.nativeTemplateEngine);ko.nativeInMemoryTemplateEngine = function (templateStorage) {
    this['templateStorage'] = templateStorage;
    this['makeTemplateSource'] = function (template) {
            // Named template
            return new ko.templateSources.inMemory(template, this['templateStorage']);
        };
}

ko.nativeInMemoryTemplateEngine.prototype = new ko.nativeTemplateEngine();
ko.nativeInMemoryTemplateEngine.instance = new ko.nativeInMemoryTemplateEngine({});
ko.setTemplateEngine(ko.nativeInMemoryTemplateEngine.instance);

ko.exportSymbol('ko.nativeInMemoryTemplateEngine', ko.nativeInMemoryTemplateEngine);(function() {
